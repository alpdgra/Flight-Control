...
- **Runways are one-way.** The chevrons on a strip show the only direction it is
  landed in. Aircraft enter at that threshold and run its whole length, rather
  than cutting in part-way, curving round the end to line up if they arrive
  side-on. Arriving from the far side costs a run down the length of the strip —
  that is the point of a one-way runway.
