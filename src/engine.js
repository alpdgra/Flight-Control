...
(function (root, factory) {
   ...
   * `angle` is the heading aircraft travel while landing.
   */
  function strip(opts) {
    ...
    return {
      ...
      tx: opts.x - dx * half, ty: opts.y - dy * half,
      ex: opts.x + dx * half, ey: opts.y + dy * half,
      // where a snapped approach begins
      fx: opts.x - dx * (half + APPROACH_LEN),
      fy: opts.y - dy * (half + APPROACH_LEN)
    };
  }
  ...
  /**
   * Snap the tail of an aircraft's path onto a zone's approach.
   *
   * The player drags *onto* the zone, so the drawn line always ends on top of
   * it — past the point where the approach begins. Those trailing points have
   * to go, otherwise the aircraft overflies the runway, doubles back to the
   * approach fix, and lands on the third pass.
   */
  Game.prototype.assignLanding = function (a, zone) {
    ...
    // Trim back to the last drawn point that is still short of the approach
    // fix, measured along the runway's own axis.
    while (a.path.length) {
      var p = a.path[a.path.length - 1];
      if ((p.x - zone.fx) * zone.dx + (p.y - zone.fy) * zone.dy > 0) a.path.pop();
      else break;
    }

    // Whatever the path now ends at may still sit ahead of the fix — the
    // aircraft itself might be past the runway. Flying straight from there to
    // the fix can mean reversing back down the centreline, so check the turn it
    // would demand; if it is too tight, go round a circuit instead.
    var tail = a.path.length ? a.path[a.path.length - 1] : { x: a.x, y: a.y };
    var ahead = (tail.x - zone.fx) * zone.dx + (tail.y - zone.fy) * zone.dy;
    var reach = dist(tail.x, tail.y, zone.fx, zone.fy);
    var joinTurn = reach < 30 ? 0
      : Math.abs(angleDelta(Math.atan2(zone.fy - tail.y, zone.fx - tail.x), zone.angle));

    if (ahead > 0 && joinTurn > MAX_JOIN_TURN) {
      var nx = -zone.dy, ny = zone.dx;
      var lat = (tail.x - zone.fx) * nx + (tail.y - zone.fy) * ny;
      var side = lat >= 0 ? 1 : -1;
      if (Math.abs(lat) < 24) {
        // Sitting on the centreline: break toward whichever side has room.
        var probeX = zone.fx + nx * CIRCUIT_OFFSET;
        var probeY = zone.fy + ny * CIRCUIT_OFFSET;
      ...
      var self = this;
      /* A point in the runway's own frame: `along` measured from the approach
         fix in the landing direction, `lateral` out to one side. */
      var placeAt = function (along, lateral) {
        return {
          x: clamp(zone.fx + zone.dx * along + nx * lateral, 12, self.W - 12),
          y: clamp(zone.fy + zone.dy * along + ny * lateral, 12, self.H - 12)
        };
      };
      ...
      var addLeg = function (pt) {
        var prev = a.path.length ? a.path[a.path.length - 1] : { x: a.x, y: a.y };
        if (dist2(pt.x, pt.y, prev.x, prev.y) > 25 * 25) a.path.push(pt);
      };
      // A full circuit: out to the side, back down past the fix, then across
      // onto the extended centreline. Every corner is a right angle, and the
      // downwind leg always has length because it runs past the fix rather
      // than stopping abeam it.
    ...
    a.path.push({ x: zone.fx, y: zone.fy });
    a.path.push({ x: zone.ex, y: zone.ey });
  };
  ...
  Game.prototype.checkTouchdown = function (a) {
    ...
    if (a.path.length <= 1) {
      var along = (a.x - z.tx) * z.dx + (a.y - z.ty) * z.dy;
      var across = Math.abs(-(a.x - z.x) * z.dy + (a.y - z.y) * z.dx);
  ...
  Game.prototype.beginLanding = function (a, z) {
    ...
    if (z.kind === 'helipad') {
      ...
      a.landDur = 0.78;
      a.heading = z.angle;
      a.drawHeading = z.angle;
      a.landFrom = { x: a.x, y: a.y };
      a.landTo = { x: z.ex, y: z.ey };
    }
    this.score++;
    ...
    for (var i = 0; i < this.aircraft.length; i++) {
      ...
      if (a.landingZone) {
        for (var z = 0; z < this.zones.length; z++) {
          if (this.zones[z].id === a.landingZone.id) { a.landingZone = this.zones[z]; break; }
        }
      }
    }
